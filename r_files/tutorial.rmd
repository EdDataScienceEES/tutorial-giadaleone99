...
To do this we will group by beach ID, then sum cig stubbs and packets then divide this by the number of surveys for that beach to obtain an average. 
...
We can see from the output that points are too big and they are indistinguishable so we can make them smaller by specifying the radius.
...
Now, in this map, you should be able to click the points and view the popup information which tells you the beach name, the county and the region as well as the average CRL count per 100m
...
If you are looking for a quicker way to do this you can try using the mapview() package which ....

Creating a spatial object 

```{r}
# Spatial analysis
beaches_obj <- st_as_sf(new_beaches, coords =  c('beach_longitude', 'beach_latitude'), crs = 4326)
```


```{r}
mapview(beaches_obj, zcol = 'average_crl')
```
